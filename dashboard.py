...
def grade_task_with_breakdown(task_id: str, actions: List[EmailAction], inbox: List[Email]) -> tuple:
    ...
    if task_id == "easy_classification":
        ...
        # Part 2: Fake Urgency Detection (20%)
        ...
        # Part 3: Priority/Time Sensitivity (15%)
        ...
        score = accuracy_contribution + fake_urgency_contribution + priority_contribution + efficiency_contribution
        
        breakdown = {
            "📧 Accuracy (50%)": f"{correct_count}/{len(inbox)} correct = {accuracy_score:.3f} → {accuracy_contribution:.3f} pts",
            "🎭 Fake Urgency (20%)": f"{fake_urgent_detected}/{fake_urgent_total} detected = {fake_urgency_score:.3f} → {fake_urgency_contribution:.3f} pts",
            "🎯 Priority (15%)": f"Urgent emails first = {priority_score:.3f} → {priority_contribution:.3f} pts",
            "⚡ Efficiency (15%)": f"Steps: {actual_steps}/{optimal_steps} optimal = {efficiency_score:.3f} → {efficiency_contribution:.3f} pts",
            "─" * 30: "",
            "🏆 TOTAL": f"{accuracy_contribution:.3f} + {fake_urgency_contribution:.3f} + {priority_contribution:.3f} + {efficiency_contribution:.3f} = {score:.3f}"
        }
